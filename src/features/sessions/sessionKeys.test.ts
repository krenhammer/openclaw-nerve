...
describe('sessionKeys', () => {
  ...
  it('builds display labels from label, displayName, then root id', () => {
    expect(getSessionDisplayLabel(session('agent:reviewer:main', { label: 'Reviewer', displayName: 'webchat:reviewer' }), 'vowel | Nerve')).toBe('Reviewer');
    expect(getSessionDisplayLabel(session('agent:reviewer:main', { displayName: 'Reviewer Prime' }), 'vowel | Nerve')).toBe('Reviewer Prime');
    expect(getSessionDisplayLabel(session('agent:reviewer:main', { label: 'Reviewer' }), 'vowel | Nerve')).toBe('Reviewer');
    expect(getSessionDisplayLabel(session('agent:reviewer:main'), 'vowel | Nerve')).toBe('Agent reviewer');
    expect(getSessionDisplayLabel(session('agent:main:main'), 'vowel | Nerve')).toBe('vowel | Nerve (main)');
  });
